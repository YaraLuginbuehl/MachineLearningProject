...
from Denoising_2D.UNET.CNN_YaraLug_UNET_CROSSVAL import UNET_FUNCTION
...
for fold_result in results:
    ...
    for i, (title, img) in enumerate(zip(titles, images)):
        ...
        plt.axis("off")
    plt.tight_layout()
    plt.suptitle(f"Model 4 - Fold {fold}")
    plt.savefig(f"/cluster/home/yluginbuehl/Denoising_2D/UNET/Model4/Model4_UNET_Image_Fold{fold}")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"/cluster/home/yluginbuehl/Denoising_2D/UNET/Model4/Model4_UNET_Loss_Fold{fold}")
