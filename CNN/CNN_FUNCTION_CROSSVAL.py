def CNN_FUNCTION(
    ...
    n_splits=5  # Add n_splits for cross-validation
):
    import numpy as np
    import torch
    ...
    class CNN(nn.Module):
        def __init__(self, activation='relu', num_layers=3):
    ...
    def validation(clean, predicted):
        ...
        for i in range(len(clean)):
            gt = clean[i, 0].astype(np.float32)  # FIX: remove .cpu().numpy()
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(indices)):
        ...
        model = CNN(activation=activation, num_layers=num_layers).to(device)
        ...
        # Get validation ground truth and noisy images
        val_noisy, val_clean = [], []
